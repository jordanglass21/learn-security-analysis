 ...
 */
router.post('/', validateBookDetailsMiddleware, appRateLimiter,  async (req: Request, res: Response) => {
  ...
  if (familyName && firstName && genreName && bookTitle) {
    try {
      ...
      res.status(500).send('Error creating book: ' + bookTitle); // false positive
    }
  } else {
